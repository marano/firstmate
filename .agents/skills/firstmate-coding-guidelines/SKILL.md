...
- Never wrap `bin/fm-test-run.sh` or that lane in `mutex`: the runner takes the machine-wide build lock itself (per serial script, per concurrent phase). `bin/fm-build-lock.sh` recognises both and stands down rather than double-holding, so a wrap no longer costs the whole lane, but write the command without it; its header owns the rule and which programs it names.
