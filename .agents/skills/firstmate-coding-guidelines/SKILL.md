...
- Never wrap `bin/fm-test-run.sh` or that lane in `mutex`: the runner takes the machine-wide build lock itself (per serial script, per concurrent phase), and `bin/fm-build-lock.sh`'s header owns the one-hold-per-run rule.
