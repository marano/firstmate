...
2. **Record a later reconcile notification for any home whose own books disagree.**
   ...
   `landing_blocked` is the one done-child case that is not that steady state: `bin/fm-awaiting-landing-lib.sh` reports the forge head that home recorded as no longer its branch's work (or as held by no repository on this machine, so it cannot be compared), so the PR would land the wrong commits and nobody should merge it.
...
- A merge card copies its PR's `held_because` from the snapshot into the card's `held_because` field whenever it is not null, and the board shows it verbatim; never author that text.
...
- Every PR waiting on the captain states why, copied from its `recorded_prs` row's `held_because`; never compose a hold reason, and never render a held PR as a bare "waiting for your word".
  A PR whose only reason is no standing merge authority on its project is a posture nobody has ruled on: render it as that fact, with the merge-authority question for that project, never as a request to approve this merge.
  A PR whose `held_because` is null has nothing on record holding it: say so rather than inventing a reason for waiting.
