...
Declare paused: before going quiet on any wait, including one the thread launched itself (bin/fm-brief.sh's rule 4 PAUSE_RULE owns the wording); never use it for a blocker that needs firstmate to act.
