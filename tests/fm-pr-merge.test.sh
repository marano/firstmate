...
# forge did not report as a boolean; both refuse before any merge call. The
# merge records through fm-pr-check.sh, whose arm-time draft refusal must stand
# aside so this merge-time refusal is the one reported and pr= is still recorded.
test_github_draft_or_unreadable_draft_state_refuses() {
  ...
  for label in draft unreadable; do
    ...
    assert_no_grep 'mark it ready for review' "$case_dir/stderr" \
