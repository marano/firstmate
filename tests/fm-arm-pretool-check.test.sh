...
test_direct_policy_contract() {
  ...
  assert_policy compound-unrelated-kill-and-watch-mention-passes allow 'if true; then kill 123; fi; echo fm-watch'
  assert_policy quoted-message-body-passes allow 'bin/fm-send.sh fm-x "kill fm-watch" && echo `date` && for i in 1; do :; done'
  assert_policy quoted-multiline-message-body-passes allow $'if x; then bin/fm-send.sh a "stop backstop: never pkill fm-watch"\nfi'
