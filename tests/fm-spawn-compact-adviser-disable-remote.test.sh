...
# The remote launch resolves its bare-name harness from the remote host's own
# tracked bin first, so the harness is installed there as well.
...
# bin/fm-spawn.sh refuses a bare-name harness missing from PATH; this suite is
# about what happens past that preflight, so every harness is installed here.
...
run_remote_launch() {  # <label>
  ...
  : > "$HERDR_LOG"
  remote_env "$ROOT/bin/fm-spawn.sh" ios --secondmate >"$TMP_ROOT/spawn.out" 2>&1 \
    || fail "$label: the remote second-mate launch failed: $(cat "$TMP_ROOT/spawn.out")"
}
