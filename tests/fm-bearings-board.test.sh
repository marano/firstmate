...
test_merge_card_carries_its_recorded_hold_reason() {
  local home data rc out
  home=$(make_home heldbecause)
  data="$home/payload.json"
  write_valid_payload "$data"
  jq '.captains_call[1].held_because = "no standing merge authority on this project; nobody has ruled"' \
    "$data" > "$data.tmp" && mv "$data.tmp" "$data"
  run_board "$home" build "$data" >/dev/null \
    || fail "a merge card carrying its recorded hold reason was refused"
  extract_payload "$home/.lavish/bearings-board.html" | jq -e '
    .captains_call[1].held_because == "no standing merge authority on this project; nobody has ruled"
  ' >/dev/null || fail "the built board dropped the merge card's hold reason"

  write_valid_payload "$data"
  jq '.captains_call[0].held_because = "not a merge"' "$data" > "$data.tmp" && mv "$data.tmp" "$data"
  set +e; out=$(run_board "$home" build "$data" 2>&1); rc=$?; set -e
  [ "$rc" -ne 0 ] || fail "a hold reason on a decision card was accepted"

  write_valid_payload "$data"
  jq '.captains_call[1].held_because = 7' "$data" > "$data.tmp" && mv "$data.tmp" "$data"
  set +e; out=$(run_board "$home" build "$data" 2>&1); rc=$?; set -e
  [ "$rc" -ne 0 ] || fail "a non-string hold reason was accepted"
  pass "a merge card carries its recorded hold reason, and only a merge card may"
}
