...
mkdir -p "$SPAWN_HOME"
# fm-spawn anchors its shared Treehouse project lock in the root home's state
# directory (fm_treehouse_project_lock_path, bin/fm-wake-lib.sh), and with no
# FM_HOME it takes FM_ROOT_OVERRIDE - this checkout - as that home. A fresh
# checkout has no state/, so a spawn case that inherits it passes only when an
# earlier script in the same run happened to leave one behind. Every spawn case
# that reaches the lock names this throwaway firstmate home instead, keeping the
# lock inside TMP_ROOT and the verdict independent of what ran before.
SPAWN_FM_HOME="$TMP_ROOT/firstmate-home"
