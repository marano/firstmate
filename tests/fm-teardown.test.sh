...
SH
  ...
  case "$mode" in
    missing-adapter|unreadable-adapter|missing-parser|missing-explicit-close-helper)
      ...
      elif [ "$mode" = unreadable-adapter ]; then
        # Present but unreadable is the other way `.` fails, and it fails the
        # same way: the adapter must be refused, not sourced and hoped for.
...
test_herdr_flat_teardown_preflight_refuses_before_changes() {
  ...
  assert_herdr_teardown_preflight_refuses_before_changes unreadable-adapter
