...
SH
  ...
  ln -s "$ROOT/bin/fm-build-lock-key-lib.sh" "$fake/bin/fm-build-lock-key-lib.sh"
  ln -s "$ROOT/bin/fm-wake-lib.sh" "$fake/bin/fm-wake-lib.sh"
  # fm-gate-refuse-lib.sh: teardown sources it before any fleet mutation.
  ln -s "$ROOT/bin/fm-gate-refuse-lib.sh" "$fake/bin/fm-gate-refuse-lib.sh"
  # fm-pr-lib.sh: teardown uses its canonical task-ID validator for poll cleanup.
  ln -s "$ROOT/bin/fm-pr-lib.sh" "$fake/bin/fm-pr-lib.sh"
  # fm-meta-keys-lib.sh: fm-pr-lib.sh sources it for the task-record key vocabulary.
  ln -s "$ROOT/bin/fm-meta-keys-lib.sh" "$fake/bin/fm-meta-keys-lib.sh"
...
SH
  ...
  # fm-github-issue-lib.sh: teardown sources it for the issue mirror's requeue move.
  ln -s "$ROOT/bin/fm-github-issue-lib.sh" "$fake/bin/fm-github-issue-lib.sh"
