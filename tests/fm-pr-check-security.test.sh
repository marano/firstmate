...
test_draft_pull_request_is_not_armed() {
  ...
  grep -qF 'is a draft pull request' "$dir/stderr" || fail "the refusal did not name the draft state"
  grep -qF 'mark it ready for review' "$dir/stderr" || fail "the refusal did not say how to clear it"
