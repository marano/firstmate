...
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/fm-brief-fixed.6nznol/state/demo-scout.status'`
...
Firstmate steers you through durable message files in '/tmp/fm-brief-fixed.6nznol/state/demo-scout.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/fm-brief-fixed.6nznol/state/demo-scout.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/fm-brief-fixed.6nznol/state/demo-scout.inbox'/NNN.msg '/tmp/fm-brief-fixed.6nznol/state/demo-scout.inbox'/handled/`.
The move IS the acknowledgement: without it firstmate rings again and eventually treats you as stuck. An empty or absent inbox needs no action.

# Definition of done
Write your findings to `/tmp/fm-brief-fixed.6nznol/data/demo-scout/report.md`.
The report must stand alone: what you did, what you found, the evidence (commands run, output, file:line references), and what you recommend.
If your deliverable is a visual artifact the captain will review and iterate on, you may host the Lavish review loop yourself (poll, revise, re-serve, staying alive) instead of handing it back to firstmate.
Before reporting done, read and follow `/Users/marano/.no-mistakes/worktrees/2d7095a62a33/01M2KKZED2R2GVEVX8MN5GSEHM/.agents/skills/captain-hold-lifecycle/SKILL.md` and pass its shared completion gate for the report and any visual review.
