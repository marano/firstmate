...
config/fleet-capacity  optional number of tasks this home runs at once, read only by the idle-fleet alarm and never enforced at dispatch; LOCAL, gitignored, and not inherited; absent means that alarm fires only on a completely idle fleet; see docs/configuration.md "Fleet capacity"
