...
config/fleet-capacity  optional number of tasks this home runs at once, read ONLY by the idle-fleet alarm as the number it compares against and never as authority: no dispatch consults it and no spawn is refused or deferred for exceeding it; LOCAL, gitignored, and not inherited; absent or malformed evaluates nothing and reports the disabled alarm once, since no default cap is honest; see docs/configuration.md "Fleet capacity"
