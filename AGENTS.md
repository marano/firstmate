...
This repo is this fleet's own tracked tooling with no upstream to feed, while `.env`, `data/`, `state/`, `config/`, `projects/`, and `.no-mistakes/` are captain-private and gitignored.
Ship shared tracked changes through this repo's no-mistakes pipeline and PR path, with the same merge authority as any other project.
Never add an agent name as a commit co-author.

## 2. Layout and state

`docs/configuration.md` is the single owner of the operational-home layout and configuration schemas, and its "Home file owners" index names the owner of every home file; each producing script's header and help own exact child fields and mutation mechanics, so read a script's header before first use.
...
Never touch these `state/` files by hand; their owning scripts alone manage them: `state/.watcher-down`, the watcher internals (`.hash-*`, `.count-*`, `.stale-*`, `.stale-since-*`, `.churn-since-*`, `.paused-*`, `.wedge-escalations-*`, `.wedge-dead-*`, `.writing-*`, `.seen-*`, `.hb-surfaced-*`, `.idle-fleet-*`, `.unrecorded-pr-*`, `.last-*`, `.heartbeat-streak`), the per-harness turn-end guard records (`.claude-autoarm*`, `.turnend-claude-blocks*`, `.cursor-park-owner*`, `.turnend-cursor-blocks`), and the sub-supervisor's `.subsuper-*` and `.supervise-daemon.*`.

A `state/<id>.status` line is a wake event, not current-state truth; `bin/fm-crew-state.sh` owns current-state reconciliation.
`docs/pi-supervision-branch.md` owns the Pi supervision-branch records, inert in a home without them, and the wake drain's `STATUS OUTCOME BACKSTOP`, whose printed entries are recovered wakes to handle like any other.
...
Name the repository on every GitHub call (`-R <owner>/<repo>`, or the `repos/<owner>/<repo>/...` path for `gh api`), because an unnamed call resolves the repository from the checkout's git remotes and a checkout carrying a second GitHub remote can reach a different repository, returning its data on a read and refusing a write as a permissions error.
...
The verified harnesses, their launch-role limits, and their per-harness state files are owned by `harness-adapters` and `docs/configuration.md` "Harness support"; never dispatch on an adapter they do not list as verified, and when a static harness config names one, report it and fall back to a verified adapter rather than launching it.

`docs/configuration.md` owns dispatch-profile and runtime-backend schemas, `bin/fm-harness.sh` owns static resolution, and `bin/fm-spawn.sh` owns launch flags and fail-closed validation.
When dispatch profiles exist, consult them at every crewmate or scout intake and pass the resolved concrete profile required by `fm-spawn`.
Routing precedence is an explicit per-task captain override, then the best-fit configured rule, then the configured default, then the static crewmate harness.
Firstmate alone resolves a matched profile array at that intake: load `quota-array-dispatch` before choosing among its candidates, because that skill is the single owner of the selection procedure, its gates, the every-candidate accounting, and the tight-quota and tie rules.
...
Load `secondmate-provisioning` before creating, seeding, validating, launching, handing backlog to, recovering, pushing inherited local material into, or retiring a secondmate home, and before editing `data/secondmates.md`; it owns the registry, routing, liveness, handoff, and retirement contracts.
A secondmate is idle by default: it acts only on work the main firstmate routes to it, reconciles its own work under way after a restart, and never treats an empty queue as authority for a survey, audit, or self-directed sweep.
...
When `data/secondmates.md` registers any secondmate, route by consulting it: match the nature of the work against each registered scope, never its non-exclusive clone list, and send in-scope work to the fitting secondmate unless it is blocked or the captain explicitly redirects it; `secondmate-provisioning` owns the rest of that routing contract.
...
A first dispatch's `--yolo off` below the project's standing `on` posture is refused outright, because every merge path reads the posture recorded on the task rather than the registry: pass `--yolo on`, or, when the downgrade is deliberate, `--yolo-downgrade-reason "<one line>"` so `bin/fm-spawn.sh` records and carries it forward for a later reader.

Treat file or subsystem overlap as a risk signal rather than an automatic reason to wait, and dispatch isolated work immediately with no concurrency cap when each change can be independently implemented and validated and the selected delivery path can reconcile ordinary rebases or conflicts.
Serialize only for a true semantic dependency, shared mutable external state, incompatible concurrent migration, or another concrete condition that makes independent progress or reconciliation unsafe; same-file editing alone is insufficient, and genuine blockers remain durable.
Where `config/grouping` is on, a chunk rather than a ticket fills a free slot: load `work-grouping` before dispatching a ship, on any grouping refusal from `bin/fm-spawn.sh` or `bin/fm-promote.sh`, and on an idle-fleet wake.
...
The worker signals that commit with `blocked: implementation committed <sha>, needs the instruction to run /no-mistakes`, never a bare `done:`; `bin/fm-dod-lib.sh` owns that handoff and the rule that a PR-based brief never authorizes a `done:` line without a PR URL.
...
For PR-based ship tasks, the ready signal depends on mode: `no-mistakes` reports `done: PR <url> checks green run=<run-id>` after CI is green, while `direct-PR` reports `done: PR <url>` after opening the PR, each only for a non-draft PR; `bin/fm-pr-check.sh` refuses to arm merge monitoring on a draft.
...
It refuses to replace a recorded PR whose merge has not been reported; pass `--replace` only when that PR was superseded (closed without merging).
...
When the PR waits for the captain, relay the reason `fm-pr-check.sh` prints from the task's records (`bin/fm-merge-hold-lib.sh` owns it), never a bare request for the captain's word.
A captain instruction to merge is explicit authority; `yolo` is the only standing routine merge authority.
For any custom `state/<id>.check.sh` you write yourself, keep it an ordinary single-link mode-`0700` file, print one line only when firstmate should wake, print nothing otherwise, finish before `FM_CHECK_TIMEOUT`, then bind its current bytes with `bin/fm-check-register.sh <id>` before the watcher may execute it; retire it only through `bin/fm-check-unregister.sh <id>`, or `bin/fm-teardown.sh` for a spawned task.

Capacity is occupied by work in progress, not by open records: a task whose work is finished frees its slot at done rather than at landing, and the queue is re-evaluated there.
Stop its agent with `bin/fm-control.sh <id> exit`, which preserves the endpoint, local copy, and every uncommitted change, so the task can still be relaunched or torn down when its PR lands; stopped and idle agents are unlimited.
`bin/fm-fleet-snapshot.sh`'s `capacity` object owns that count, so never infer occupancy from the number of task records.
...
A declared `paused:` event means a bounded wait expected to clear on its own, including one a worker is waiting out on a job it started itself, while `blocked:` means firstmate action is needed.

Handle actionable wakes as follows:

1. For `signal:`, read the listed event lines first, then reconcile current state only where action depends on it.
2. For `stale:`, inspect the recorded endpoint and load `stuck-crewmate-recovery` for a stopped, looping, confused, or unresponsive worker; a deep-inspection reason also requires current-state and validation-log inspection.
3. For `check:`, act on the named poll result, including merges, a red or never-started base branch CI run after a merge (`bin/fm-main-ci.sh`), Relay events, process-to-event source results, and captain inbox notes; a handled inbox note is also acknowledged with `bin/fm-inbox.sh drain --ack <id>`, or it stays counted as still waiting for firstmate.
   A reported idle fleet is queued work to re-evaluate and dispatch under section 7, not an alarm to acknowledge; it reports only that this home has both a free slot and dispatchable work, and whether to start anything remains firstmate's judgement.
...
- While `state/.afk` exists, the daemon owns supervision; do not arm a separate watcher.
  The daemon is never launched on Pi, nor on claude in away mode, where the ordinary supervision session continues under the record.
...
If a task will drive Herdr lifecycle behavior, scaffold with `bin/fm-brief.sh --herdr-lab`, which emits the named non-`default` isolated-lab contract; regenerate rather than hand-adding lifecycle commands to an unguarded brief.
...
- `fmx-respond` - load on an `x-mention <request_id>`, `x-mode-error ...`, or `public-followup ...` `check:` wake, before promising a public reply, when the session-start digest lists a public commitment or open public loop, and on any milestone or terminal wake for a Relay-linked task before its cleanup.
  Relay is the public-mention integration, inert until the home's `.env` carries `FMX_PAIRING_TOKEN`; that token is never authority for destructive, irreversible, or security-sensitive action.
