...
- Claude registers two `Stop` hooks in `.claude/settings.json`, both anchored through `CLAUDE_PROJECT_DIR`: `bin/fm-turnend-guard.sh --claude`, and `bin/fm-claude-stop-autoarm.sh` with `asyncRewake: true` and `timeout: 28800`.
  It registers the same auto-arm on `StopFailure` as `bin/fm-claude-stop-autoarm.sh --stop-failure`, because Claude runs `StopFailure` instead of `Stop` when an API error ends a turn; without it, a rewake whose handling turn failed left no watcher and no live hook.
  No guard is registered there, because Claude ignores a `StopFailure` hook's exit status unless the hook is `asyncRewake`.
...
When none of those proofs appears, it re-blocks, and `FM_CLAUDE_TURNEND_BLOCK_BUDGET` (default 3, below Claude's 8-block override; `fm_turnend_block_budget` in `bin/fm-wake-lib.sh`) bounds the continuations of one failure episode, which runs from its first charged continuation until positive watcher recovery.
...
The first stop that finds the budget spent allows the stop with the episode's one loud attended alarm, a `systemMessage` naming the supervision need and the repair, unless a final check under the locks finds a healthy watcher or an open auto-arm claim, in which case it steps aside silently.
Budget exhaustion is the whole proof, with no wait for the auto-arm to record its own failure, because an auto-arm that hangs mid-arm never records one; `tests/fm-turnend-guard.test.sh` covers the hung-claim case.
...
The Stop auto-arm reads the same budget (`fm_turnend_block_count`), because while it keeps claiming generations the guard allows every stop and never reaches its own alarm.
After its one failure notice, each later auto-arm failure continues only while the budget lasts, naming why the turn exists, and the firing that finds the budget spent delivers the episode's one attended alarm itself.
Both participants create the alarm marker exclusively under the auto-arm owner lock, so the alarm is delivered once per episode.
After it, the Stop auto-arm suppresses further exit-2 continuations and later unhealthy stops are allowed without re-blocking until positive watcher recovery.
...
Neither Claude Stop hook ever exits 2 with an empty message, because a continuation with nothing to act on only loops: every continuation names its reason, and a stop that finds a healthy watcher is allowed even when the episode reset cannot take its lock, leaving the reset for the next verified-healthy boundary.
