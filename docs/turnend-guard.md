...
Budget exhaustion is the whole proof, with no wait for the auto-arm to record its own failure, because an auto-arm that hangs mid-arm never records one: on 2026-09-18 auto-arms stalled behind a pegged `syspolicyd` for more than 80 minutes while the guard re-blocked until Claude's own 8-block override.
