...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`), scout, secondmate-charter, and Herdr-lab briefs, with Captain's intent and Firstmate spec subsections on ship/scout, and `--delivers` rendering one intent slot per delivered item beside the membership the dispatch checks |
...
| `fm-test-run.sh`         | Behavior-test runner: selection, portable lanes, bounded concurrency, budgets, coverage guard, timing/JSON; takes the machine-wide build lock once per serial script and once per whole concurrent phase; refuses to execute in the repository primary checkout when `FM_TASK_ID` marks a task worker |
| `fm-test-isolation-proof.sh` | Concurrent isolation harness and portable candidate set owner |
| `fm-build-lock.sh`       | Machine-wide build/test semaphore: run at most N local builds or tests at once (N=1 unless the machine configures more) in arrival order, hold every slot for a run named as needing the whole machine, stand down on CI, pass a nested invocation through inside an existing hold, and report a hold or wait that overruns, to the task's status file as well as stderr; `bin/mutex` is its bare-word entry point |
...
| `fm-claude-stop-autoarm.sh` | Claude Stop and StopFailure `asyncRewake` hook owning tokenless watcher continuity with single-flight exit-2 rewake (docs/watcher-continuity.md) |
...
| `fm-spawn.sh`            | Spawn crewmates, scouts, `id=repo` batches, and secondmates on the resolved harness and runtime backend, guarding a ship dispatch against starting beside related work, cleared by a recorded `--apart-reason` |
...
| `fm-task-inbox-lib.sh`   | Single owner of durable steering-inbox records, acknowledgement, doorbells, and the ladder over delivery attempts and busy runs |
...
| `fm-build-lock-key-lib.sh` | Single owner of the key prefix of the build lock's queue wait, shared by the lock and the wait classifier |
...
| `fm-tmux-lib.sh`         | Shared tmux pane primitives for composer capture, verified submit, the submit-time busy check, and the copy-mode input-ready guard |
