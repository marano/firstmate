...
| `fm-build-lock.sh`       | Machine-wide build/test mutex: run one local build or test at a time, stand down on CI, and report a hold that overruns; `bin/mutex` is its bare-word entry point |
