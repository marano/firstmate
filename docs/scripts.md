...
| `fm-build-lock.sh`       | Machine-wide build/test mutex: run one local build or test at a time in arrival order, stand down on CI, pass a nested invocation through inside an existing hold, and report a hold or wait that overruns, to the task's status file as well as stderr; `bin/mutex` is its bare-word entry point |
