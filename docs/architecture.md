...
A held build slot is evidence that WORK is progressing and never that an AGENT is attached to it, so the two are established separately before either can hold or fire an escalation.
A quiet pane whose worker has positively ended its turn - an exact `idle` semantic busy verdict, never merely the absence of `busy` - while a live build-slot holder sits in the worktree recorded for that task is classified as its own condition rather than as a declared wait or a possible wedge.
That condition holds the wedge ladder for as long as the holder is live, leaving the escalation count untouched and re-surfacing only on the `FM_PAUSE_RESURFACE_SECS` cadence with a reason naming the holder's pid, command and elapsed time, because escalating it ends in a relaunch that would kill a real run mid-test and suppressing it would silence a true alarm.
The moment the run that took that hold is gone while the turn is still over, the same condition surfaces at once instead of on any cadence, because from then on a finished result is sitting unobserved and the cost of the condition is the time it spends uncollected.
The run is the holder's ancestors inside the holder's own process group, because one run can take many holds: a per-script runner such as `bin/fm-test-run.sh`, or a worker's own loop of `mutex` runs, holds nothing between two scripts or while its next one queues for a slot, and is held as the same live run until those ancestors have ended.
A holder whose group is its terminal's foreground group records no run, because an agent that runs commands in its own group would otherwise be read as a run that never ends, so there the holder alone decides.
A declared wait is separately classified by whether a supervisor can resolve what it names: a pid, a build-slot holder, a run id, a URL, a path or a UTC clearing time is checkable, while a harness-internal job id is not.
That verdict annotates the declared wait's own bounded recheck reason rather than changing its cadence, because the cadence has its own owner and an unnamed subject is not grounds to take it away, and a captain-held transfer is never annotated since the subject of that wait is the captain rather than a job.
The annotation is what lets a supervisor tell `waiting on build slot, pid 45757` from `waiting on background test run bhymd1si9` without asking the worker, so an unverifiable wait is never read as evidence that the work still exists.
Holder attribution is by the cwd the hold was taken in, anchored to the recorded worktree or a path under it, and every negative - a free machine, a holder with no recorded cwd, a dead holder's surviving record, a prefix-sharing sibling worktree, a secondmate home, a missing worktree record, or a lock read that outlives its bound - leaves the existing escalation schedule untouched.
An adapter whose busy contract cannot produce a positive `idle`, such as Codex, standalone Kimi, Rovo and AGY, falls through this classification untouched and keeps the ordinary ladder, since declining to claim a finished turn costs a late notice while guessing one would hold a real wedge off the ladder indefinitely.
...
A home with a free task slot and dispatchable queued work is a detected condition rather than a resting state.
Both of the triggers that re-evaluate the queue can disappear at once: teardown re-evaluates it only after work LANDS, so nothing merging suppresses it entirely, and the periodic fleet review is a `heartbeat` wake, which the away-mode daemon force-self-handles by design, so with both gone nothing anywhere asks whether to start something.
The watcher therefore compares two numbers it can already reach on its own bounded `FM_IDLE_FLEET_SCAN_SECS` cadence: this home's task records whose latest status line has not declared a concluded outcome, and `tasks-axi ready`'s count of work dispatchable right now.
Counting concluded workers as free capacity is the load-bearing part, because a delivered crew still holding an unmerged branch is an idle agent rather than work under way, and reading the backlog's own In-flight rows instead would have shown a full fleet throughout the incident this detector exists for.
Once both hold continuously for `FM_IDLE_FLEET_SECS` the watcher appends one keyed `check` wake naming all three numbers, re-surfacing it no more often than `FM_IDLE_FLEET_RESURFACE_SECS` while the condition persists and retiring the episode the moment it clears.
It is hosted in the watcher rather than in the away-mode daemon's own fleet scan because that daemon runs only in the away and quiet postures while the attended fleet has the same blind spot, and because a `check` is escalated by the daemon rather than self-handled, which is what keeps a stopped fleet from being absorbed as routine progress.
Detection ends there: nothing is dispatched, no backlog item is transitioned, and what to start remains firstmate's judgement under [`../AGENTS.md`](../AGENTS.md) section 7.
An unreadable capacity disables the detector and says so as its own wake, while an unreadable ready count is left to `bin/fm-bootstrap.sh`'s existing missing-tool diagnostic rather than reported twice.
`bin/fm-idle-fleet-lib.sh`'s header owns the condition and each of its three numbers, [`configuration.md`](configuration.md) owns `config/fleet-capacity`, and `tests/fm-idle-fleet.test.sh` pins the condition matrix, the sustain window, the re-surface cadence, deduplication, the refusals, and the away-mode escalation.
An open PR on a task's branch while the task's record carries no `pr=` is also a detected condition, because `bin/fm-pr-check.sh` records a PR and arms its merge poll only when firstmate handles the worker's ready line, and a worker that opens a PR and goes quiet otherwise leaves it unwatched.
On its own bounded `FM_UNRECORDED_PR_SCAN_SECS` cadence the same watcher compares this home's ship records that have a branch and no recorded PR against one forge query per sweep, skipped entirely when no record is a candidate, and appends one keyed `check` wake per finding naming the task and the PR URL, re-surfacing a handled finding no more often than `FM_UNRECORDED_PR_RESURFACE_SECS` while it holds.
It is hosted beside the idle-fleet detector for the same reason, so the away daemon escalates it; it never records the PR or arms a poll itself, because recording binds a task to a forge identity that firstmate must verify first.
`bin/fm-unrecorded-pr-lib.sh`'s header owns the condition, and `tests/fm-unrecorded-pr.test.sh` pins the silent cases, the single query, the dedupe and re-surface, the binding boundary, and the away-mode escalation.
...
A task that [`bin/fm-awaiting-landing-lib.sh`](../bin/fm-awaiting-landing-lib.sh) reports as awaiting landing skips stale triage entirely in both postures, so its quiet pane raises no stale wake and holds no wedge timer or escalation count, whether its agent was stopped deliberately or left idle; that library owns the state, and a task that leaves it is classified afresh.
...
While a `paused:` declaration is still a crew's last status line and its agent is confirmed alive, an active no-mistakes run's `working` verdict confirms that declared wait rather than superseding it - a crew's own no-mistakes round is one of the bounded waits `paused:` names - so the pane keeps the long `FM_PAUSE_RESURFACE_SECS` cadence instead of arming the short wedge timer; a dead agent still falls through to the ordinary wedge-eligible `working` classification.
...
On Pi, pi-signed, and claude the away daemon is no longer launched: the ordinary supervision session continues under the record, which on claude is the Stop-hook asyncRewake cycle that wakes firstmate without typing into its composer.
A presence-gated sub-supervisor (`bin/fm-supervise-daemon.sh`) still extends this for walk-away supervision on the other harnesses, and for `/quiet` on claude: the `/afk` skill starts it through the tracked foreground helper `bin/fm-afk-start.sh` once the record exists, after which the watcher reverts to daemon-managed one-shot mode and the daemon self-handles routine wakes in bash.
...
That kind also carries a trailing sentinel, so a digest cut from the front reads as corrupt provenance rather than as the captain returning, and the daemon's shutdown retains undelivered escalations instead of typing them into a pane it can no longer watch.
...
A busy pane is never typed into either, but busy defers the doorbell rather than stopping the ladder: an unbroken busy run carrying an unhandled record past the ladder's own bound surfaces as a worker that cannot be reached, which is what distinguishes one held on a blocking harness prompt from one merely working slowly (`bin/fm-task-inbox-lib.sh` header).
A composer holding text that never submits is four different conditions, not one, and that same header owns the model separating them, along with the measured evidence that no mechanism available through the control plane clears the worst of them.
A positively idle pane whose composer provably holds exactly firstmate's own doorbell has Enter re-pressed on it automatically; a busy pane and an unreadable one are left completely untouched, because the record is durable and interrupting a live turn to deliver a message that was arriving anyway is worse than the defect; text firstmate never typed is never submitted or cleared, since it belongs to someone else; and a doorbell firstmate typed that never cleared escalates as a relaunch, which two live incidents showed is the only recovery.
...
`bin/fm-backend.sh` centralizes backend selection, `state/<id>.meta` helpers, metadata-only cleanup identity validation, selector resolution, and operation dispatch; `bin/backends/tmux.sh` is the verified reference adapter ([`docs/tmux-backend.md`](tmux-backend.md)), `bin/backends/herdr.sh` (P2) has its own CI lane, off unless `FM_CI_RUN_HERDR` is `true` ([`docs/herdr-backend.md`](herdr-backend.md)), and `bin/backends/zellij.sh` (P3), `bin/backends/orca.sh` (P4), and `bin/backends/cmux.sh` (P5) remain experimental task-spawn adapters with no dedicated real-backend CI lane.
...
Green is not validated, so on either forge the verified head must also be proven by the no-mistakes pipeline's own run record, never by a pull request body a worker can edit, and a `direct-PR` or `local-only` task has no such run by definition.
Without that proof the merge is refused unless an attended `--unvalidated` carries an explicit captain instruction to merge without a validation run; that waiver is refused while away and is separate from `--attended-override`, so following a merge-queue retry hint never waives validation, and `bin/fm-pr-merge.sh`'s header owns the evidence rule.
The candidate runs are bound to the pull request being merged rather than to the lane's newest activity, because every earlier source went stale the moment a lane shipped a second pull request.
A receipt captured when `bin/fm-pr-check.sh` binds the pull request keeps a validated head provable after the pipeline stops answering for its run, and it answers only where that run's own record cannot be read at all, so a record that still answers always decides.
[`bin/fm-validation-receipt-lib.sh`](../bin/fm-validation-receipt-lib.sh)'s header owns the candidate rule, the record's proof rule, the receipt format, and the receipt's exact trust boundary.
Because away merge authority is read from the away-posture record and then acted on by the forge, the authority read and synchronous forge command share that record's cross-subsystem lock, closing the common live-owner TOCTOU.
...
Only after that confirmed landing, the script deletes the pull request's own head branch itself rather than relying on forge auto-delete settings, skipping deletion (and reporting why, without failing the run) whenever it cannot identify the branch, the branch lives in a fork, is also the base branch, is protected, or is the base of another open pull request or merge request; `bin/fm-pr-merge.sh`'s header owns the full skip-condition list.
A repository whose own auto-delete setting is on removes the head ref within seconds of the merge, inside the window those guard reads occupy, so a delete that fails is followed by one more read of the branch: a branch that is already gone reached the outcome the step wanted and is reported as gone rather than as a failure.
A branch that is still there after its delete failed is recorded in the fleet-wide `state/branch-orphans` record instead of being reported once on stderr and forgotten, and the next merge on that same repository retries exactly the branches recorded for it; [`bin/fm-branch-orphan-lib.sh`](../bin/fm-branch-orphan-lib.sh) owns the record and [`bin/fm-branch-orphans.sh`](../bin/fm-branch-orphans.sh) owns the sweep, which re-reads every condition live because a record can be days old and a deletion is irreversible.
