...
The Herdr CI lane, skipped unless the repository variable `FM_CI_RUN_HERDR` is `true`, uses the pinned installers in `bin/fm-install-herdr.sh` and `bin/fm-install-treehouse.sh`.
