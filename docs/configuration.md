...
Completion closes an item only when something shows its work was started - a status line, a pull request, or an unpushed commit on its local copy - and otherwise returns it to Queued with the reason in its body, so cleaning up a worker that never ran is never recorded as shipped work.
