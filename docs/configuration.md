...
Spawn likewise refuses, before creating any endpoint or local copy, a harness whose resolved command is not executable on this host, naming the harness and command ([`bin/fm-spawn.sh`](../bin/fm-spawn.sh)).
...
`bin/fm-brief.sh --delivers` scaffolds the matching brief, with each delivered item's own ask in its own slot, and the dispatch refuses a brief whose recorded membership differs from its own or whose slots are still unfilled.
...
`tmux` is the verified reference backend (see [`docs/tmux-backend.md`](tmux-backend.md)); `herdr` has its own CI lane, off unless `FM_CI_RUN_HERDR` is `true` (see [`docs/herdr-backend.md`](herdr-backend.md)); `zellij`, `orca`, and `cmux` remain experimental spawn backends with no dedicated real-backend CI lane (see [`docs/zellij-backend.md`](zellij-backend.md), [`docs/orca-backend.md`](orca-backend.md), and [`docs/cmux-backend.md`](cmux-backend.md)).
...
Target detection uses `FM_SUPERVISOR_TARGET`, then `$TMUX_PANE`, then `"${HERDR_SESSION:-default}:${HERDR_PANE_ID}"` under herdr, and there is no fallback target.
When none of those resolve, away-mode entry refuses and the daemon refuses at startup, each naming every marker it looked for.
A guessed pane that happens to exist passes the daemon's startup validation, so the alternative is a home that looks away while every escalation is deferred into a pane that never ran firstmate.
...
Where the posture is `warn` or `enforce`, a ship dispatch - `bin/fm-spawn.sh`, and `bin/fm-promote.sh` at the moment a scout becomes a ship worker - is guarded against starting work beside related work: another ready item under the same repository and key, a live worker already on that group, planned members the dispatch does not name, or an item carrying no key while other ready or live ship work shares its repository.
Each finding prints the command that resolves it; under `warn` they print and the dispatch proceeds, and under `enforce` they refuse before any endpoint, local copy or record exists.
`--apart-reason "<one line>"` clears the whole guard in one command and is recorded on the task record and on the item, so a dispatch that deliberately went alone stays auditable after cleanup; it is refused on a restart, a scout, a secondmate, and a batch.
A restart of an existing worker is never guarded, because its membership and its reason are already recorded.
...
The value is a property of the home doing the dispatching, so it is not inherited by secondmate homes, which run their own crew mix.
...
The shell scripts do not match those rules; firstmate chooses the best matching rule with judgment, resolves its profile object or array under the `quota-array-dispatch` skill, and passes only concrete `--harness`, `--model`, and `--effort` flags to `fm-spawn.sh`.
...
A runner still waiting out the replaced generation's floor sees the replacement within a quarter second and exits without launching, so it does not hold the source against the fresh generation's first launch for the rest of that floor.
...
FM_CLAUDE_STOPFAILURE_BACKOFF=60   # seconds the StopFailure auto-arm waits before re-arming after an API-error turn end, doubling per consecutive API-error turn end; a completed turn resets it
FM_CLAUDE_STOPFAILURE_BACKOFF_MAX=900   # cap in seconds on that doubled StopFailure backoff
...
FM_CLAUDE_TURNEND_BLOCK_BUDGET=3   # continuations one Claude supervision failure episode may force (guard re-blocks and Stop auto-arm retries) before its one attended alarm ends them; safely below Claude Code's 8-block override
...
FM_WATCHER_EXIT_LOCK_TIMEOUT=5   # seconds a closing watcher waits for the recovery-marker lock before exiting with its singleton lock retained for the successor's reclaim (docs/watcher-continuity.md)
FM_SIGNAL_GRACE=30      # seconds to coalesce nearby status and turn-end signals into one wake; also how long a bounded checkpoint waits after its deadline TERM before killing the watcher (docs/watcher-continuity.md)
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; stale panes whose crew is not provably working surface immediately unless admitted directly to the declared-wait cadence, while a live idle declared wait still surfaces once, worded as that wait, before that cadence bounds repeats
...
FM_IDLE_FLEET_SCAN_SECS=300        # how often the idle-fleet condition is evaluated at all; the ready count costs a backlog subprocess, so it does not ride the poll cadence
FM_IDLE_FLEET_SECS=900             # how long a free slot plus dispatchable queued work must hold CONTINUOUSLY before the first alarm, so an ordinary gap between one task landing and the next dispatch stays silent
FM_IDLE_FLEET_RESURFACE_SECS=3600  # how often a condition that keeps holding re-surfaces, so one missed or deferred alarm cannot buy another long silence; see config/fleet-capacity above for the capacity the condition compares against
FM_UNRECORDED_PR_SCAN_SECS=300     # how often the watcher compares this home's branch-bearing ship records that have no recorded PR against one forge query for open PRs; no query runs when no record is a candidate
FM_UNRECORDED_PR_RESURFACE_SECS=3600   # how often an open PR that is still unrecorded after firstmate handled its notification is raised again
FM_UNRECORDED_PR_TIMEOUT=20        # seconds that one forge query may take before the sweep gives up and logs it instead of waking anyone
...
FM_TASK_SHELL_LOCK_BIN=bin/fm-build-lock.sh   # test override for the build-lock binary `crew_task_shell_running` reads via `--holders` to attribute a live build-slot holder to a quiet task's own worktree
FM_TASK_SHELL_TIMEOUT=10    # wall-clock seconds that one `--holders` read may take inside a supervisor poll; hitting the bound reads as no evidence, like the worktree write probe's own bound above; a value that is not a positive integer falls back to the default
