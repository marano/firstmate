...
An absent file is refused on the same footing, so this alarm is off until the home writes the file.
There is no honest number to assume for a home that has not stated its cap: [`../AGENTS.md`](../AGENTS.md) section 7 sets no fleet-wide concurrency cap, a home's real cap lives in its own private captain preferences as prose, and a built-in default of 1 would quietly make the alarm deaf in every home that runs more than one task at a time, since the in-progress count can then never fall below the capacity while any work is under way.
Either refusal disables the alarm and reports itself once as a supervision notification naming the file, so a home never runs with a detector it cannot read and no word of it.
