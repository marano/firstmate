...
| interrupted install recovery | the same suite holds a public bind where its package is renamed into the content-addressed store but not yet sealed `0555`, kills the installer there, confirms the kill left an unsealed directory and the dead installer's lifecycle lock, and proves the next bind recovers that lock, reinstalls over the directory, seals it, and verifies; a second bind of the same package started while the first is held there waits on the lifecycle lock and ends with the ordinary already-bound refusal, never a refusal of the first bind's unsealed directory |
