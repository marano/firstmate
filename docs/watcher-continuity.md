...
The same hook is registered on `StopFailure`, which Claude runs instead of `Stop` when an API error ends a turn, so a rewake whose handling turn fails still re-arms; the script header owns its bounded backoff and stand-down rules.
...
Claude's Stop hook starts the successor arm at the next Stop, or `StopFailure`, after the handling turn, rather than before notification as Pi, omp, and OpenCode do.
...
It sends that TERM again halfway through the wait, because Bash 5.2 can lose a watcher's trapped signal ("Startup-path substitutions" in `bin/fm-wake-lib.sh`), and a holder that lost the first one keeps beating, so the stale-beacon kill below never applies and the restart would attach to the watcher it meant to replace.
...
An arm torn down by a signal stops the watcher it started the same way, then kills it if it outlives the whole wait, and the foreground checkpoint kills a watcher that outlives its deadline's TERM by a short grace; a watcher killed that way leaves its lock to the successor's stale-lock reclaim, which publishes the downtime first.
...
It also covers a restart that waits out a default-poll sleep instead of attaching to the watcher it stopped, a stale TERM-surviving holder that a restart kills and a plain arm leaves alone, a holder that beat during the stop wait and is spared, a fresh holder that lost its first TERM and ends on the re-sent one, an arm teardown that kills a child surviving both TERMs, the bounded exit-path marker-lock wait with its successor recovery, and the poll-derived arm grace.
Three of its cases, which run on Bash 5 and later, signal a shell spinning on the pid probe, on the startup lock and recovery-marker path, and on the stray-owner collection every free acquire runs, and fail as soon as one pending trap is lost.
`tests/fm-watch-checkpoint.test.sh` covers a checkpoint that kills a watcher surviving its deadline's TERM and still returns.
...
That suite also drives the tracked `StopFailure` registration after an API-error turn end, the backoff that doubles to its cap and resets on a completed turn, and the stand-down of a waiting firing when a turn completes or a newer failure arrives.
`FM_CLAUDE_LIVE_E2E=1 tests/fm-claude-stop-autoarm-live-e2e.test.sh` starts with the reproduced stale-lock state, runs session start first, completes two tokenless cycles, and checks the competing-live-owner negative control.
`FM_CLAUDE_LIVE_E2E=1 tests/fm-claude-stopfailure-rearm-live-e2e.test.sh` cuts the network for a rewake's handling turn and proves the `StopFailure` registration re-arms and wakes the idle session again.
...
Claude depends on the Stop and `StopFailure` `asyncRewake` rewake, Cursor depends on its awaited stop-hook park, Grok retains native background-completion notifications, and Codex retains bounded foreground checkpoints.
