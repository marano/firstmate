...
Two of its cases, which run on Bash 5 and later, signal a shell spinning on the pid probe and on the startup lock and recovery-marker path, and fail on Bash 5.2 as soon as one pending trap is lost.
