...
|---|---|---|
| Ship child PR ready | the child's `done: PR <url> ...` line; `pr=` in the child's record once registered | `bin/fm-inactive-reconcile.sh` on the next poll with the child's line; `bin/fm-pr-check.sh` at first registration with the canonical URL and, when the PR waits for the captain, the recorded reason (`bin/fm-merge-hold-lib.sh`) |
