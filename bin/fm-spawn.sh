...
if [ "$KIND" = ship ] || [ "$KIND" = scout ]; then
  ...
  # bin/fm-build-lock.sh appends its ceiling lines here; its header owns that use.
  spawn_send_text_line "$T" "export FM_TASK_STATUS=$(shell_quote "$STATE_REAL/$ID.status")"
fi
