...
tests/fm-cursor-harness.test.sh	incompat:no longer incompatible - its faked process name is a symlink to bash, not a copy, so macOS no longer SIGKILLs it and it passes under stock bash; stays excluded only until the macOS lane split decides membership
...
tests/fm-harness-precedence.test.sh	incompat:no longer incompatible - its faked process name is a symlink to bash, not a copy, so macOS no longer SIGKILLs it and it passes under stock bash; stays excluded only until the macOS lane split decides membership
...
tests/fm-muse-harness.test.sh	incompat:no longer incompatible - its faked process name is a symlink to bash, not a copy, so macOS no longer SIGKILLs it and it passes under stock bash; stays excluded only until the macOS lane split decides membership
