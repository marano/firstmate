...
# fm-alert-lib.sh - backend-independent active alerts to the captain.
# Usage: . bin/fm-alert-lib.sh   (the embedder defines log and FM_HOME)
#
# The single owner of the active-alert channels configured by config/wedge-alarm
# (docs/wedge-alarm.md owns the operator reference). Two embedders share them:
# the away-mode daemon's injection wedge alarm (bin/fm-supervise-daemon.sh) and
# the independent supervision-dead checker (bin/fm-supervision-alert.sh).
# WEDGE_ALARM_TITLE is the notification title the osascript and herdr channels
# show; an embedder sets it before wedge_alarm_notify, and it defaults to the
# away-mode wedge alarm's title.
# The library itself never defaults the FM_WEDGE_ALARM_EXEC seam: each embedder
# owns its own library-mode guard so a sourced test context cannot post a real
# notification.
...
# A tmux status-line flash is a cosmetic, client-side OSD with no cross-backend
# equivalent, so a wedged non-tmux primary (the 2026-07-10 overnight incident: a
# claude-on-herdr primary) got NO active signal - only a passive durable marker,
# which nothing surfaces until the next fleet action (that night, 20 escalations
# sat buffered for 8.5h). These helpers add a configurable active alert that does
# not depend on any pane or its backend status-line: an OS-level macOS
...
# and is skipped, never crashing the embedder - and the embedder's own durable
# record stays the primary signal whether or not any channel fires.
#
...
# so no test can post a real desktop notification - each embedder's library-mode
# guard (the foot of bin/fm-supervise-daemon.sh and bin/fm-supervision-alert.sh)
# defaults it to "discard" whenever that embedder is SOURCED rather than
# executed. The special value "discard" fires nothing; unset means production
# (an executed embedder), so the real channels fire.
...
# source) so their text can never break the script. Best-effort: logs and returns 1 on failure.
...
# channel failure can never abort the embedder. Any
