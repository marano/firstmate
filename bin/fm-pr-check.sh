...
# A GitHub pull request the forge reports as a draft is refused before anything
# is recorded or armed, naming the draft state: a draft cannot be merged, so a
# poll armed on it would wait for an event that cannot occur while nobody is
# asked to merge. Mark the pull request ready for review, then arm again; a lane
# that keeps a draft on purpose declares a wait instead of reporting done.
# An unreadable draft state does not refuse, matching the optional head read.
...
# Only a positive draft reading refuses; an unreadable one must not block arming.
if [ "$PROVIDER" = github ] && [ "${FM_PR_CHECK_MERGE:-}" != 1 ] \
  ...
  if [ "$(fm_pr_json_draft_state "$DRAFT_JSON")" = true ]; then
    echo "error: $URL is a draft pull request; a draft cannot be merged, so merge monitoring would wait for an event that cannot occur - mark it ready for review (gh-axi pr ready) and arm again, or declare a wait instead of done if the draft is deliberate" >&2
    exit 1
  fi
fi
