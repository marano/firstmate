...
fm_run_timed() {  # <seconds> <command...>
  ...
  case "$(fm_timeout_mechanism)" in
    ...
    perl)
      perl -e 'my $t = shift; my $pid = fork; die "fork failed" unless defined $pid; if (!$pid) { setpgrp(0, 0); exec @ARGV } local $SIG{ALRM} = sub { kill "TERM", -$pid; select undef, undef, undef, 0.2; kill "KILL", -$pid; exit 124 }; alarm $t; waitpid $pid, 0; print STDERR "FM_DEBUG_PERL_RAW_STATUS=$?\n" if $ENV{FM_DEBUG_REAP_DIR}; exit($? >> 8)' \
        "$seconds" "$@"
      ;;
