...
- ask-user findings are never yours to answer: escalate to firstmate using rule 6's ask-user format and stop.
  ...
  A firstmate decision answers only the run and findings its \`nm-<run>-<step>\` key names; a finding in any other run, including a rerun raising the same finding, is a new ask-user gate to escalate as a new needs-decision, never one to answer from an earlier decision.
