...
# Primary config/grouping travels for the same reason: a secondmate dispatches
# crew under the captain's own rules about what work belongs together, so a
# posture that holds in the primary home holds in its mates' homes too.
# It also pushes
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context launch-env-allowlist claude-permission-mode grouping}"
