...
# ready-signal shape (`done: PR <url>`, `done: PR <url> checks green`, or the
# no-mistakes DOD's `done: PR <url> checks green run=<run-id>`), so a PR a
# worker merely mentioned in prose is never claimed as the delivery.
...
pr_for_task() { # <meta> [preferred-line]
  ...
  if [ -z "$value" ] && [ -n "$preferred" ]; then
    value=$(printf '%s\n' "$preferred" \
      | sed -nE 's|^done: PR (https?://[^[:space:])"]+/pull/[0-9]+)( checks green)?( run=[^[:space:]]+)?$|\1|p' \
      | head -1 || true)
  fi
  clean_field "$value"
}
