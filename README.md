...
| `/ask`             | Present your open decisions through the interactive question picker instead of plain text, batched into one picker call, most impactful first - an opt-in that finishes all other fleet work first and refuses while you are away, because the picker pauses supervision until you answer and a second picker call in the same invocation wedges it |
...
- [docs/wedge-alarm.md](docs/wedge-alarm.md) - configure the active alert for a stuck away-mode escalation delivery, and install the launchd alert that tells you when supervision has gone silent.
